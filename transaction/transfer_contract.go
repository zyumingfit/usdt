...
func TransferContract(airDropAddress []byte, fileAudit string, fileTx string) error {
	log.Logger().Info("Create Transfer Contract begin.")
	...
	for {
		...
		if err != nil {
			log.Logger().Error(fmt.Sprintf("Decode hex airdrop address failed, reasons: [%v]\n", err))
			failed++
			continue
		}

		transferContract := &tronCore.TransferContract{
			OwnerAddress:    airDropAddress,
			ToAddress: to_address,
			Amount: interest,
		}
		// Trigger smart contract.
		transaction, err :=  utils.TronClient().CreateTransaction2(context.Background(), transferContract)
		if err != nil {
